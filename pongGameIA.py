...
# Cargar el modelo de la red neuronal entrenado
model = tf.keras.models.load_model('pong_model.h5')

# Función para actualizar la pantalla
def draw():
    screen.fill(BLACK)
    pygame.draw.line(screen, WHITE, (320, 0), (320, 640))
...
while running:
    ...
    # Obtener la entrada de la red neuronal
    input_data = np.array([paddle1_pos, ball_pos[0], ball_pos[1]])
    input_data = input_data.reshape((1, -1))

    # Obtener la acción de la red neuronal
    action = model.predict(input_data)[0]
    if action > 0.5 and paddle1_pos > 0:
        paddle1_pos -= 5
    if action <= 0.5 and paddle1_pos < HEIGHT - PADDLE_HEIGHT:
        paddle1_pos += 5
    ...
    if ball_pos[0] >= WIDTH - PADDLE_WIDTH - BALL_RADIUS and paddle2_pos <= ball_pos[
        1] <= paddle2_pos + PADDLE_HEIGHT:
        ball_vel[0] = -ball_vel[0]
